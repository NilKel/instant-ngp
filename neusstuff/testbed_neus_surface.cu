...
__global__ void compute_loss_kernel_train_nerf_with_global_movement(
	...
	// Pure feature accumulation for pure modes (direct sum, no alpha blending)
	Eigen::Array<float, 15, 1> pure_surface_features_ray = Eigen::Array<float, 15, 1>::Zero();
	Eigen::Array<float, 15, 1> pure_volume_features_ray = Eigen::Array<float, 15, 1>::Zero();
	...
	for (; compacted_numsteps < numsteps; ++compacted_numsteps) {
		...
		if (configuration == 4) { // dual mode
			...
			// Feature accumulation for feature-based divergence regularization
			// Extract 15D surface features from rows 3-17 (dual mode) or rows 6-20 (dual_merge mode)
			// Extract 15D volume features from rows 18-32 (dual mode) or rows 21-35 (dual_merge mode)
			for (int d = 0; d < 15; ++d) {
				int surface_feat_row = (configuration == 4) ? (3 + d) : (6 + d);  // dual: 3-17, dual_merge: 6-20
				int volume_feat_row = (configuration == 4) ? (18 + d) : (21 + d);  // dual: 18-32, dual_merge: 21-35
				
				float surface_feature = float(local_network_output[surface_feat_row]);
				float volume_feature = float(local_network_output[volume_feat_row]);
				
				// Alpha-blended accumulation for feature_direct/feature_gradual modes
				surface_features_ray[d] += weight * surface_feature;
				volume_features_ray[d] += weight * volume_feature;
				
				// Pure accumulation for pure_direct/pure_gradual modes (direct sum, no weight)
				pure_surface_features_ray[d] += surface_feature;
				pure_volume_features_ray[d] += volume_feature;
			}
		} else if (configuration == 5) { // dual_merge mode
			...
			// Feature accumulation for feature-based divergence regularization (same as dual mode)
			// Extract 15D surface features from rows 6-20 (dual_merge mode)
			// Extract 15D volume features from rows 21-35 (dual_merge mode)
			for (int d = 0; d < 15; ++d) {
				int surface_feat_row = 6 + d;  // dual_merge: 6-20
				int volume_feat_row = 21 + d;  // dual_merge: 21-35
	...
	for (uint32_t j = 0; j < compacted_numsteps; ++j) {
		...
		if (divergence_loss_mode > 0 && (configuration == 4 || configuration == 5)) { // 1 = direct, 2 = gradual, 3 = feature_direct, 4 = feature_gradual, 5 = pure_direct, 6 = pure_gradual
				float lambda_reg = 0.0f;
				
				if (divergence_loss_mode == 1 || divergence_loss_mode == 3 || divergence_loss_mode == 5) { // direct modes
					lambda_reg = divergence_lambda;
				} else if (divergence_loss_mode == 2 || divergence_loss_mode == 4 || divergence_loss_mode == 6) { // gradual modes
				...
				if (lambda_reg > 0.0f) {
					...
					} else if (divergence_loss_mode <= 4) { // Alpha-blended feature modes (3 = feature_direct, 4 = feature_gradual)
						// L_reg = λ_reg * | ||F_surface||₂ - ||F_volume||₂ | where F are the alpha-blended 15D features
					...
					} else { // Pure feature modes (5 = pure_direct, 6 = pure_gradual)
						// L_reg = λ_reg * | ||F_surface_pure||₂ - ||F_volume_pure||₂ | where F are the directly summed 15D features
						// Compute L2 norms of the 15D pure feature vectors
						float pure_surface_norm = 0.0f;
						float pure_volume_norm = 0.0f;
						
						// Compute ||F_surface_pure||₂
						for (int d = 0; d < 15; ++d) {
							pure_surface_norm += pure_surface_features_ray[d] * pure_surface_features_ray[d];
						}
						pure_surface_norm = sqrtf(pure_surface_norm);
						
						// Compute ||F_volume_pure||₂  
						for (int d = 0; d < 15; ++d) {
							pure_volume_norm += pure_volume_features_ray[d] * pure_volume_features_ray[d];
						}
						pure_volume_norm = sqrtf(pure_volume_norm);
						
						// Pure feature-based divergence loss: difference of L2 norms (direct sum features)
						float pure_feature_divergence_loss = lambda_reg * fabsf(pure_surface_norm - pure_volume_norm);
						loss_output[i] += pure_feature_divergence_loss / (float)n_rays;
					}
				}
			}
		}
...
void Testbed::train_nerf_step(uint32_t target_batch_size, uint32_t n_rays_per_batch, uint32_t* counter, uint32_t* compacted_counter, float* loss, float* ek_loss, float* mask_loss, cudaStream_t stream) {
	...
	nullptr,
	 ...
	 m_divergence_loss == std::string("feature_gradual") ? 4 :
	 m_divergence_loss == std::string("pure_direct") ? 5 :
	 m_divergence_loss == std::string("pure_gradual") ? 6 : 0),
	m_divergence_lambda	);
