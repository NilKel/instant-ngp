...
/** @file   surface_normal_network.h
 *  @brief  Surface rendering with encoded analytical normals as additional input
 */
...
template <typename T>
class SurfaceNormalNetwork : public SurfaceNetwork<T> {
public:
	using json = nlohmann::json;

	SurfaceNormalNetwork(
		uint32_t n_pos_dims,
		uint32_t n_dir_dims,
		...
		this->m_method = "surface_normal";
		
		// RGB network input: 16 surface features + encoded view dirs + encoded normals
	...
	// Override inference to add normal encoding
	void inference_mixed_precision_impl(
		...
		// Encode normals too
		uint32_t normal_start_idx = 16 + this->m_dir_encoding->padded_output_width();
		auto normal_out = rgb_network_input.slice_rows(normal_start_idx, this->m_dir_encoding->padded_output_width());
		
		tcnn::GPUMatrixDynamic<float> normals_for_encoding{3, batch_size, stream, analytical_normals.layout()};
	...
	// Override forward to add normal encoding context
	std::unique_ptr<tcnn::Context> forward_impl(
		...
		// Normal encoding
		uint32_t normal_start_idx = 16 + this->m_dir_encoding->padded_output_width();
		auto normals_section = forward->rgb_network_input.slice_rows(normal_start_idx, this->m_dir_encoding->padded_output_width());
		
		tcnn::GPUMatrixDynamic<float> normals_for_encoding{3, batch_size, stream, forward->analytical_normals.layout()};
		...
		forward->normal_encoding_ctx = this->m_dir_encoding->forward(
			stream,
			normals_for_encoding,
			&normals_section,
			use_inference_params,
...
private:
	struct ForwardContext : public SurfaceNetwork<T>::ForwardContext {
		std::unique_ptr<tcnn::Context> normal_encoding_ctx;
	};
};
